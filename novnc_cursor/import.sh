...
# VNC Lab - novnc_cursor Import Script
# Import Docker images and configurations for Cursor IDE VNC environment
...
import_image() {
    ...
    done < <(docker images --format "{{.Repository}}:{{.Tag}}" | grep -E "(vnc-cursor|vnc-cursor-gpu)")
    ...
    if docker load -i "$actual_file"; then
        ...
        docker images --format "table {{.Repository}}\t{{.Tag}}\t{{.Size}}\t{{.CreatedAt}}" | grep -E "(vnc-cursor|vnc-cursor-gpu)"
