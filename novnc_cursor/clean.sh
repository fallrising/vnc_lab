...
# VNC Lab - novnc_cursor Clean Script
# Clean Docker containers and images for Cursor IDE VNC environment
...
clean_containers() {
    ...
    done < <(docker ps -a --format "{{.Names}}" | grep -E "(vnc-cursor|$CONTAINER_NAME)")
...
clean_volumes() {
    ...
    done < <(docker volume ls --format "{{.Name}}" | grep -E "(cursor|vnc)")
