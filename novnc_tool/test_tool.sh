...
# VNC Lab - novnc_tool Test Script
# Test enhanced tools functionality and container health
...
show_usage() {
    ...
    echo "  --tools                 Test development tools"
    ...
    echo "  $0 --tools              # Test development tools"
...
if [[ "$TEST_CONTAINER" == false ]] && [[ "$TEST_HOST" == false ]] && [[ "$TEST_TOOLS" == false ]]; then
    TEST_CONTAINER=true
    TEST_HOST=true
    TEST_TOOLS=true
fi
...
test_image() {
    ...
    if ! docker images "$IMAGE_NAME:latest" | grep -q "$IMAGE_NAME"; then
        print_warning "Tool image '$IMAGE_NAME:latest' not found!"
        print_status "You can build it using: ./build.sh"
        return 1
    fi
    
    print_success "Tool image exists"
...
test_development_tools() {
    print_status "Testing development tools functionality..."
    ...
    # Check Firefox
    if docker exec "$CONTAINER_NAME" which firefox &> /dev/null; then
        print_success "Firefox is installed"
        FIREFOX_VERSION=$(docker exec "$CONTAINER_NAME" firefox --version 2>/dev/null || echo "Version not available")
        print_status "Firefox version: $FIREFOX_VERSION"
    else
        print_warning "Firefox is not installed"
    fi
    
    # Check if tools processes are running
    print_status "Checking tools processes..."
    ...
    if docker exec "$CONTAINER_NAME" ps aux | grep -v grep | grep -q firefox; then
        print_success "Firefox process is running"
    else
        print_warning "Firefox process is not running"
    fi
...
# Main test execution
print_status "Starting Tool tests..."
...
# Test development tools if requested
if [[ "$TEST_TOOLS" == true ]]; then
    test_development_tools
fi

print_success "Tool tests completed!"
...
if [[ "$TEST_TOOLS" == true ]]; then
    print_status "  - Development Tools: $(test_development_tools >/dev/null 2>&1 && echo 'OK' || echo 'FAILED')"
fi 